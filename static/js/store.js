...
class Store {
  ...
  async loadProducts() {
    const productsResponse = await api.makeRequest('/stripe/products');
  ...
  async createOrder(currency, items, email, shipping) {
    try {
      const response = await api.makeRequest('/stripe/orders', {
        method: 'POST',
  ...
  async payOrder(order, source) {
    try {
      const response = await api.makeRequest(`/stripe/orders/${order.id}/pay`, {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
  ...
  async getOrderStatus(orderId) {
    try {
      const response = await api.makeRequest(`/stripe/orders/${orderId}`);
